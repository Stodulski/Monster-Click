...
let monsterMaxHp = parseInt(monsters[random].vida * (myLevel * 4));
...
function cursor() {
    document.body.onmousedown = () => {
        document.body.style.cursor =
            "url('/public/img/png/cursorAtack.png'), auto";
    };
...
function spawnMonster() {
    ...
    setTimeout(() => {
        ...
        monsterSpawn.setAttribute(
            'src',
            'public/img/png/' + monsters[random].nombre
        );

        monsterExp = monsters[random].exp * multiplicador;
        monsterMaxHp = parseInt(monsters[random].vida * (myLevel * 4));

        actualMonsterHp = monsterMaxHp;
        antiInspec();

        monsterHp.style.background =
            'linear-gradient(180deg, rgba(0,231,50,1) 0%, rgba(0,255,58,1) 50%, rgba(0,191,37,1) 100%)';
